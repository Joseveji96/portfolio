...
const Hero: React.FC<HeroProps> = ({ opacity, translateY }) => {
	...
	return (
		<section className='w-screen text-textColor  md:bg-red-400'>
			<motion.div
				style={{ opacity, translateY, transform: translateY }}
				className='flex flex-col justify-center items-center gap-2'
			>
				<motion.div variants={slideUp2} initial="initial" animate="enter" className='font-dm font-bold text-5xl sm:text-6xl xl:text-8xl 2xl:text-9xl 3xl:text-[10rem] uppercase tracking-wide px-4 sm:px-5 md:px-24 3xl:px-36'>
					<h1 className='pl-0 text-center sm:text-left'>EVERY PROJECT</h1>
					<div className='flex flex-row justify-center items-center sm:pl-6 md:pl-28'>
						<h1 className='text-center sm:text-left'>Can be</h1>
						<motion.div variants={slideUp} initial="initial" animate="enter" className="mx-4 sm:mx-6 md:mx-10 my-4 sm:my-0">
							<Image
								ref={imageRef}
								src={"/flor.svg"}
								alt={'Flor'}
								width={100}
								height={100}
								className="block w-14 md:w-32 3xl:w-48"
							/>
						</motion.div>
				...
				<div className='px-4 md:px-40 xl:px-36 font-dm flex flex-col sm:flex-row gap-4 md:gap-20 3xl:gap-32'>
					<div className='hidden sm:flex flex-col gap-1 pt-3 md:pl-28 text-center sm:text-right font-normal text-lg sm:text-base xl:text-2xl 3xl:text-3xl'>
						...
						<h2 className='sm:-translate-x-10 md:-translate-x-44'>Let´s Transform <span className='font-extrabold text-[#524848]'>constraints</span> </h2>
						<div>
			...
			{/* Descargas */}
			<div className="h-auto sm:h-10 flex flex-row justify-center md:justify-between sm:gap-6 items-center px-4 md:px-28 mt-10 ">
				<div className="flex flex-row gap-4 sm:gap-6 md:gap-16 items-center">
					<button className="font-inter text-xs md:text-[18px] font-extrabold hover:underline decoration-1 sm:decoration-2 flex gap-1 items-center">
						Download CV
				...
				<div className='flex flex-row gap-8 sm:gap-20 items-center'>
					...
					<motion.div
						...
						className="mt-0 hidden md:flex justify-center"
					>
						<motion.span
							animate={{ y: [0, 10, 0] }}
